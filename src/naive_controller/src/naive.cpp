...
void Naive_Controller::car_position_cb(const custom_msg::car_position::ConstPtr& p) {
	...
	currentIndex = getControl(currentIndex, state); //camera
	//currentIndex = getControl(currentIndex,ekf_state); //kalman
	if (stopcounter < 2000)
	...
	control.reflinvel	= u_opt(0);
	control.refangvel   = u_opt(1);
